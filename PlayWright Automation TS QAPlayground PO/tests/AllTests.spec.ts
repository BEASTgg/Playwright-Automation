...
test("Check correct link", async function({page, request}){
    ...
    if (link){                                      //check for null or gives red mark warning
...
test("Check the slider", async function({page}){
    ...
    if (property){                                                                  //need to check for null or it gives red lines
...
test("Checking sortable Menu #AlternateMethod", async function({page}){
    ...
    if (InitialBox && TargetBox){                                   //check for null or gives red mark warning
...
test("Checking Resizable Menu #1", async function({page}){
    ...
    if (ObjectBox){                                         //check for null or gives red mark warning
...
test("Checking Droppable Menu #3", async function({page}){
    ...
    if (DragBoxDimention && OuterDropBoxDimention){                     //check for null or gives red mark warning
...
test("Checking Droppable Menu #3P2", async function({page}){
    await page.goto("https://demoqa.com/droppable");
    await page.locator("#droppableExample-tab-preventPropogation").click();
    const DragBoxLocator = page.locator("#dragBox");
    const DragBoxDimention = await DragBoxLocator.boundingBox();
    ...
    if (DragBoxDimention && InnerDropBoxDimention){
        await page.mouse.move(DragBoxDimention.x + DragBoxDimention.width/2 , DragBoxDimention.y + DragBoxDimention.height/2);
        await page.mouse.down();
        await page.mouse.move(InnerDropBoxDimention.x + InnerDropBoxDimention.width/2 , InnerDropBoxDimention.y + InnerDropBoxDimention.height/2);
        await page.mouse.up();
    }
    const OuterStatusMsg = await page.locator("//div[@id = 'notGreedyDropBox']/p").textContent();
    const InsideStatusMsg = await page.locator("//div[@id = 'notGreedyInnerDropBox']/p").textContent();
    expect(OuterStatusMsg).toBe("Dropped!");
    expect(InsideStatusMsg).toBe("Dropped!");
});

//Cannot check Y since it changed when it reverted back to original position.
test("Checking Droppable Menu #4", async ({page}) => {
    await page.goto("https://demoqa.com/droppable");
    await page.locator("#droppableExample-tab-revertable").click();
    const willRevertLocator = page.locator("//div[@class='drag-box mt-4 ui-draggable ui-draggable-handle']").nth(4);
    const willRevertDimention = await willRevertLocator.boundingBox();
    const targetLocator = page.locator("#droppable").last();
    await willRevertLocator.dragTo(targetLocator);
    const status = await page.locator("//div[@id = 'droppable']/p").last().textContent();
    await expect(status).toBe("Dropped!");
    const MovedButtonLocator = page.locator("//div[@class='drag-box mt-4 ui-draggable ui-draggable-handle']").nth(4);
    const NewPositionDimention = await MovedButtonLocator.boundingBox();
    if (willRevertDimention && NewPositionDimention){
        await expect(willRevertDimention.x).toEqual(NewPositionDimention.x);
        await expect(willRevertDimention.width).toEqual(NewPositionDimention.width); //No sense of checking width as it doesnt changes
        //await expect(willRevertDimention.y).toBeCloseTo(NewPositionDimention.y);  // Not checking y as it changes a little bit when it moves.
        await expect(willRevertDimention.height).toEqual(NewPositionDimention.height); //No sense of checking height as it doesnt changes
        //await expect(willRevertDimention).toEqual(NewPositionDimention);
    }
});

test("Checking Droppable Menu #4P2", async function({page}){
    await page.goto("https://demoqa.com/droppable");
    await page.locator("#droppableExample-tab-revertable").click();
    const willNotReverLocator = page.locator("#notRevertable");
    const willNotReverDimention = await willNotReverLocator.boundingBox();
    const targetLocator = page.locator("#droppable").last();
    await willNotReverLocator.dragTo(targetLocator);
    const status = await page.locator("//div[@id = 'droppable']/p").last().textContent();
    await expect(status).toBe("Dropped!");
    const MovedButtonLocator = page.locator("#notRevertable");
    const NewPositionDimention = await MovedButtonLocator.boundingBox();
    if (willNotReverDimention && NewPositionDimention){
        await expect(willNotReverDimention.x).not.toEqual(NewPositionDimention.x);
        await expect(willNotReverDimention.width).toEqual(NewPositionDimention.width);
        await expect(willNotReverDimention.y).not.toBeCloseTo(NewPositionDimention.y);
        await expect(willNotReverDimention.height).toEqual(NewPositionDimention.height);
    }
});

test("Checking dragable Menu #1", async function({page}){
    await page.goto("https://demoqa.com/dragabble");
    const initial = page.locator("#dragBox");
    const targetx = page.locator("#draggableExample-tab-containerRestriction");
    await initial.dragTo(targetx);
});

test("Checking dragable Menu #2", async function({page}){
    await page.goto("https://demoqa.com/dragabble");
    await page.locator("#draggableExample-tab-axisRestriction").click();
    const xAxisLocator = page.locator("#restrictedX");
    const xAxisDimention = await xAxisLocator.boundingBox();
    if (xAxisDimention){
        await page.mouse.move(xAxisDimention.x + xAxisDimention.width/2 , xAxisDimention.y + xAxisDimention.height/2);
        await page.mouse.down();
        await page.mouse.move(xAxisDimention.x + xAxisDimention.width/2 + 50, xAxisDimention.y + xAxisDimention.height/2 + 50);
        await page.mouse.up();
    }
    const xAxisLocatorNew = page.locator("#restrictedX");
    const xAxisDimentionNew = await xAxisLocatorNew.boundingBox();
    if (xAxisDimention && xAxisDimentionNew){
        await expect(xAxisDimention.y).toBe(xAxisDimentionNew.y);
        await  expect(xAxisDimention.x).not.toBe(xAxisDimentionNew.x);
    }
});

test("Checking dragable Menu #2P1", async function({page}){
    await page.goto("https://demoqa.com/dragabble");
    await page.locator("#draggableExample-tab-axisRestriction").click();
    const yAxisLocator = page.locator("#restrictedY");
    const yAxisDimention = await yAxisLocator.boundingBox();
    if (yAxisDimention){
        await page.mouse.move(yAxisDimention.x + yAxisDimention.width/2 , yAxisDimention.y + yAxisDimention.height/2);
        await page.mouse.down();
        await page.mouse.move(yAxisDimention.x + yAxisDimention.width/2 + 50 , yAxisDimention.y + yAxisDimention.height/2 + 50);
        await page.mouse.up();
    }
    const yAxisLocatorNew = page.locator("#restrictedY");
    const yAxisDimentionNew = await yAxisLocatorNew.boundingBox();
    if (yAxisDimention && yAxisDimentionNew){
        await expect(yAxisDimention.x).toBe(yAxisDimentionNew.x);
        await expect(yAxisDimention.y).not.toBe(yAxisDimentionNew.y);
    }
});

test("Checking dragable Meny #3", async ({page}) => {
    await page.goto("https://demoqa.com/dragabble");
    await page.locator("#draggableExample-tab-containerRestriction").click();
    const DraGLocator = page.locator("//div[@class = 'draggable ui-widget-content ui-draggable ui-draggable-handle']");
    const DraGDimention = await DraGLocator.boundingBox();
    const OuterContainerLocator = page.locator("#containmentWrapper");
    const OuterContainerDimention = await OuterContainerLocator.boundingBox();
    if (DraGDimention){
        await page.mouse.move(DraGDimention.x + DraGDimention.width/2 , DraGDimention.y + DraGDimention.height/2);
        await page.mouse.down();
        await page.mouse.move(DraGDimention.x + DraGDimention.width/2 + 500 , DraGDimention.y + DraGDimention.height/2 + 500);
        await page.mouse.up();
    }
    const DraGLocatorNew = page.locator("//div[@class = 'draggable ui-widget-content ui-draggable ui-draggable-handle']");
    const DraGDimentionNew = await DraGLocatorNew.boundingBox();
    if (DraGDimentionNew && OuterContainerDimention){
        await expect(DraGDimentionNew.x + DraGDimentionNew.width).toBeLessThanOrEqual(OuterContainerDimention.x + OuterContainerDimention.width);
        await expect(DraGDimentionNew.y + DraGDimentionNew.height).toBeLessThanOrEqual(OuterContainerDimention.y + OuterContainerDimention.height);
    }
});